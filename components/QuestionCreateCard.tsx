...
const QuestionCreateCard: React.FC<Props> = ({ addQuestion }) => {
  ...
  const toggleCorrectOption = (id: number) => {
      setOptions(options.map(option => option.id === id ? { ...option, correct: !option.correct } : option));
  };
      ...
      <div className="p-4 shadow rounded bg-white">
          ...
          {options.map((option, index) => (
              <div key={index} className="flex items-center mb-2">
                  ...
                  />
                  <button onClick={() => toggleCorrectOption(option.id)} className={`ml-2 ${option.correct ? 'bg-green-500' : 'bg-gray-500'} text-white p-2 rounded`}>
